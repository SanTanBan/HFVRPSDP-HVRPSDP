...
def HLS_Text(directory_details_for_saving="",directory_containing_Vehicle_Types_file="",directory_containing_Node_Locations_file="",directory_containing_Distance_Matrix_file=""):

    os.mkdir(directory_details_for_saving+"HLS Text Solution")
    directory_to_save_HLS_solution=directory_details_for_saving+"HLS Text Solution/"
    ...
    def Decoding_Mechanism(Node_Sequence):
        ...
        # The Solution is represented as set of only those X(i,j,k)==1, where i to j is the traversed From and To Nodes by a Vehicle Type K just like in PuLP
        return Min_Cost_Array[0][0],message,solution,Num_of_Vehicle_of_each_Type_being_used

    Node_Seq,Latitude,Longitude,PickUps,Deliveries=GeneRead.Reader.Relief_Centre_Reader(directory_of_Relief_Centre_Specifications_file=directory_containing_Node_Locations_file)
    ...
    # Shuffling the initial Node Sequence
    Depot_First_Node=Node_Seq[0]
    Node_Sequence=Node_Seq[1::+1]
    random.shuffle(Node_Sequence)
    Node_Sequence.insert(0,Depot_First_Node)
    ...
    while f<f_iter:
        ...
        if x_dash[0]<=t*x_b[0]:
            ...
            if len(Tabu_List)==50: # Length of the Tabu list is hereby limited to 50 elements
                del Tabu_List[0]
                ii-=1 ### This is also added wrt the HLS algorithm file (needs to be seen whether this may be commented out)
            Tabu_List.append(x)
            if x[0]<x_b[0]:
                f=0
                x_b=x
            ...
            f+=1 ### This is added wrt the HLS Algorithm File [ The HLS Flowchart is incorrect i.e. the Paper Flowchart is incorrect]
            age+=1
            if age==N:
                age=0
                t=t+a1*a2
