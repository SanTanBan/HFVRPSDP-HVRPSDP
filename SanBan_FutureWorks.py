...
def SANBAN_FutureWork(max_seconds_allowed_for_calculation=86400,directory_containing_Node_Specifications_file="",directory__containing_Vehicle_Specifications_file="",directory_containing_Distance_Matrices_file="",directory_details_for_saving=""):
    
    os.mkdir(directory_details_for_saving+"SanBan Future Work")
    directory_to_save_Gurobi_solution=directory_details_for_saving+"SanBan Future Work/"
    ...
    for i in Depot_and_Relief_Centres:
        for j in Depot_and_Relief_Centres:
            if i!=j:
                y[i, j] = mdl.addVar(vtype=GRB.CONTINUOUS, name="x%d,%d" % (i, j))
                mdl.update()
    ...
    for i in Depot_and_Relief_Centres:
        for j in Depot_and_Relief_Centres:
            if i!=j:
                z[i, j] = mdl.addVar(vtype=GRB.CONTINUOUS, name="x%d,%d" % (i, j))
                mdl.update()
    ...
    #Ensuring at most a single vehicle caters to a Relief Center (Eq. 2)
    ...
    # Ensuring at most VN outgoing paths are available at the Depot since there are VN[k] vehicle for each Vehicle Type (Eq. 4)
    mdl.addConstrs(quicksum(x[0,j,k] for j in Relief_Centres) <= VN[k] for k in Vehicle_Types)
    mdl.update()

    """Flow Limitation Constraints"""

    #Ensuring initial PickUp from Nodes is 0 (Eq. 42)
    mdl.addConstrs(y[0,j] == 0 for j in Relief_Centres)
    mdl.update()

    #Ensuring final Delivery to Nodes is 0 (Eq. 43)
    mdl.addConstrs(z[i,0] == 0 for i in Relief_Centres)
    mdl.update() 

    #Ensuring the PickUp constraints are satisfied (Eq. 44)
    mdl.addConstrs((quicksum(y[i,j]- y[j,i] for j in Depot_and_Relief_Centres if i!=j)) == PickUp[i] for i in Relief_Centres)
    mdl.update()

    #Ensuring the Delivery constraints are satisfied (Eq. 45)
    mdl.addConstrs((quicksum(z[j,i] - z[i,j] for j in Depot_and_Relief_Centres if i!=j)) == Delivery[i] for i in Relief_Centres)
    mdl.update()
    ...
    # Ensuring the vehicle capacity is never exceeded (Eq. 46)
    mdl.addConstrs(y[i,j]+z[i,j] <= quicksum(VQ[k]*x[i,j,k] for k in Vehicle_Types) for i in Depot_and_Relief_Centres for j in Depot_and_Relief_Centres if i!=j )
    mdl.update()
    ...
    if Solutions_Found:
        ...
        for k in Vehicle_Types:
            ...
            for i, j in routes:
                utilized_capacity=y[i,j].x+z[i,j].x
