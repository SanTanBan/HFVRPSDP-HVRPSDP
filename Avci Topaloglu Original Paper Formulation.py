...
def Avci_Exact_Algorithm(max_seconds_allowed_for_calculation=86400):
    ...
    for k in Vehicle_Types:
        temp = pd.read_excel("Network Distance Matrix.xlsx","Distances for VT "+str(k))
        for i,row in temp.iterrows():
            key=(int(row["Origin Node"]),int(row["Destination Node"]))
            value=float(row["Distance"])
            C[key]=value
        break
    del temp

    #print("The set of Vehicle is:- ",set_of_Vehicles)
    #print(C)
    ...
    mdl.setObjective(quicksum(VC[k[0]]*x[0,j,k] for k in set_of_Vehicles for j in Relief_Centres) + quicksum(VS[k[0]]*x[i,j,k]*C[i,j] for k in set_of_Vehicles for i in Depot_and_Relief_Centres for j in Depot_and_Relief_Centres),GRB.MINIMIZE)
    mdl.update()
